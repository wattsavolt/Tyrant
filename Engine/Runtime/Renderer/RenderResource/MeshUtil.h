...
namespace tyr
{
	...
	public:
		...
		// Same as CreateNormalsAndTangents, but leaves vertex.normal untouched - use this
		// when normals are already valid (e.g. authored/imported) and only tangents are
		// missing. Same accuracy note as above applies, since the underlying math is the same.
		static void CreateTangents(Vertex* vertices, uint numVertices, const uint* indices, uint numIndices);
