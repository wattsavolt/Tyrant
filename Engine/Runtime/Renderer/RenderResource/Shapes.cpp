...
namespace tyr
{
	...
	const ShaderVertex* Cube::GetShaderVertices() const
	{
		return m_ShaderVertices.Data();
	}
	...
	Sphere::Sphere(float radius, uint16 latLines, uint16 longLines)
	{
		...
		m_ShaderVertices.Resize(numVertices);
		...
		for (uint16 i = 0; i < latLines - 2; ++i)
		{
			...
			for (uint16 j = 0; j < longLines; ++j)
			{
				...
				m_Vertices[index].uv.x = Math::Atan2(currVertPos.x, currVertPos.z) * Math::c_InvTwoPi + 0.5f;
				m_Vertices[index].uv.y = currVertPos.y * 0.5f + 0.5f;
			}
		}
		...
		MeshUtil::CreateShaderVertices(m_Vertices.Data(), m_ShaderVertices.Data(), numVertices);
	}

	const Vertex* Sphere::GetVertices(uint& size) const
	{
		size = m_Vertices.Size();
		return m_Vertices.Data();
	}

	const ShaderVertex* Sphere::GetShaderVertices(uint& size) const
	{
		size = m_ShaderVertices.Size();
		return m_ShaderVertices.Data();
	}

	const uint* Sphere::GetIndices(uint& size) const
	{
		size = m_Indices.Size();
